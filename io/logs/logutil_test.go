...
func TestConfigurePersistantLogging(t *testing.T) {
	...
	err := ConfigurePersistentLogging(fmt.Sprintf("%s/%s/%s", testParentDir, existingDirectory, logFileName), logrus.InfoLevel)
	require.NoError(t, err)

	// 2. Test creation of file along with parent directory
	nonExistingDirectory := "test-2-non-existing-testing-dir"

	err = ConfigurePersistentLogging(fmt.Sprintf("%s/%s/%s", testParentDir, nonExistingDirectory, logFileName), logrus.InfoLevel)
	require.NoError(t, err)
	...
	err = ConfigurePersistentLogging(fmt.Sprintf("%s/%s/%s/%s", testParentDir, existingDirectory, nonExistingSubDirectory, logFileName), logrus.InfoLevel)
	require.NoError(t, err)
